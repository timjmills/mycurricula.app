...
browser before "done", and a pass across all six themes (see "Cross-theme
verification" below).

**Cross-theme verification (mandatory).** Every new surface or feature must
be verified across all six themes — `paper` (default), `cloud`, `night`
(dark mode), `mint`, `sky`, `blossom` — the same way the §4 responsive
contract requires the three viewport tiers. Switch themes via Settings →
Appearance (instant, persisted per device) and confirm on each: chrome
(nav/tabs/rail icons/filter chips) re-hues through the `--chrome-accent-*`
tier rather than staying a stale brand color; no surface goes dark-on-dark
or light-on-light; text holds WCAG AA contrast. Night plus at least one
wash is the floor for a focused change; an app-wide wave covers all six.
...
4. Responsive testing. Resize the window to mobile (375px), tablet (768px),
   ...
   content.
5. Theme testing. Switch through all six themes (`paper`, `cloud`, `night`,
   `mint`, `sky`, `blossom`) via Settings → Appearance and screenshot each
   affected surface. Flag any theme where chrome (nav/tabs/rail/chips) fails
   to re-hue, text drops below WCAG AA contrast, a surface stays dark-on-dark
   or light-on-light, or a token reads wrong. Night (the dark mode) and at
   least one wash are mandatory; covering all six is the bar.
6. Report. Write findings to QA-REPORT.md with: severity
...
375/768/1440 widths sit inside the §4 responsive tiers, and the six-theme
sweep mirrors the §4 theme axis — this audit supplements the §4 contract,
...
- **Verify across all six themes.** Before declaring a screen done, switch
  through `paper`, `cloud`, `night`, `mint`, `sky`, and `blossom` (Settings →
  Appearance). Confirm: chrome re-hues through the `--chrome-accent-*` tier
  (no stale brand color), no surface goes dark-on-dark or light-on-light, and
  text holds WCAG AA contrast — especially on Night. See §4 (theme axis) and
  §4b (cross-theme verification) for the full rule.
...
**How to trigger it.** The `gen-image` skill (`.claude/skills/gen-image/SKILL.md`)
is the full runbook — invoke it via the Skill tool / `/gen-image`. The underlying
one-liner is:

```powershell
$null | codex exec --sandbox workspace-write --skip-git-repo-check '$imagegen <subject>, <style>, <aspect ratio>. Save the file to <ABSOLUTE_PATH>.png'
```

Two Windows must-dos: `$null |` is mandatory (Codex hangs on stdin otherwise — the
PowerShell equivalent of `< /dev/null`), and single-quote the prompt so `$imagegen`
reaches Codex literally. Use aspect ratios (16:9 / 1:1 / 9:16), never pixel sizes;
the absolute save path goes in the prompt (the tool has no save-path flag) and must
be inside the repo workspace.

**Folder conventions.**

- UI/UX mockups → `design/mockups/<feature>/` — generate → read back → critique →
  implement; pass a screenshot as a `-i` reference for redesign variations.
- Website images → `public/` (the Next.js static dir) — generate a PNG, then convert
  to web size/format with `node .claude/skills/gen-image/to-web.mjs` (uses the
  bundled `sharp`; prefer webp/jpeg; 1200×630 og-image option).
- Default brand style: clean, education/classroom, teacher-focused, warm paper
  aesthetic; per-request override allowed.

**Active backend.** OpenAI Codex CLI v0.137.0 built-in `image_gen` tool
(gpt-image-2), authenticated via the user's ChatGPT subscription — NO API key.
Enabled via `[features] image_generation = true` in
`%USERPROFILE%\.codex\config.toml`. A `@ramlyburger/gpt-image-2-mcp` MCP server
(chatgpt-web mode) is a documented but inactive fallback.

**Caveats.** ~30s–2min per image; image generation burns ChatGPT-plan usage ~3–5×
faster than a text turn; clean `~/.codex/generated_images/` periodically.
