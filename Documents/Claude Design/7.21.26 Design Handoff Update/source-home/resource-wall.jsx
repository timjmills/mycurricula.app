...
function Section({ sec, view, layout, q, filter, dragging, cardDragging, onCardDragState, onForce, onOpen, onEnlarge, onBoard, onModal, onAdd, onAddSection, onDropCard, onDragStartSec, onDropSec, onDragEndSec, onSolo }){
  ...
  const tagLessons=(()=>{ const m=new Map(); (sec.items||[]).forEach(r=>(r.lessons||[]).forEach(l=>{ if(l&&l.title&&!m.has(l.id))m.set(l.id,l.title); })); return [...m.values()]; })();
  let items = sec.items;
  ...
  return (
    <section className={'rw-sec'+(bg?' has-bg':'')} data-size={size} style={{'--sc':cv(sec.color), ...bgStyle, ...(dragging?{transition:'all .18s ease'}:{})}}
      ...
      <div className="rw-sechead" draggable onDragStart={e=>{ if(e.target.closest('.rw-secbgpop')||e.target.closest('.rw-sectags')){ e.preventDefault(); return; } e.dataTransfer.setData('text/sec',sec.id); e.dataTransfer.effectAllowed='move'; onDragStartSec&&onDragStartSec(sec.id); }}
        onDragEnd={()=>onDragEndSec&&onDragEndSec()}
        onClick={(e)=>{ if(e.target.closest('.rw-secactions')||e.target.closest('.rw-grip')||e.target.closest('.rw-collapse')||e.target.closest('.rw-sectags')) return; cycle(); }}>
        ...
        {tagLessons.length>0 && <span className="rw-sectags" onClick={e=>e.stopPropagation()}>
          {tagLessons.slice(0,3).map((t,i)=><span key={i} className="rw-sectag" title={'Tagged to '+t}>{W.link}<span className="rw-sectaglbl">{t}</span></span>)}
          {tagLessons.length>3 && <span className="rw-sectagwrap">
            <button className="rw-sectagmore" title="Show all tagged lessons" onClick={(e)=>{e.stopPropagation();setTagsOpen(o=>!o);}}>+{tagLessons.length-3}</button>
            {tagsOpen && <div className="rw-sectagpop" onMouseLeave={()=>setTagsOpen(false)}><div className="rw-sectagpoph">Tagged lessons</div>{tagLessons.map((t,i)=><span key={i} className="rw-sectagrow">{W.link}{t}</span>)}</div>}
          </span>}
        </span>}
        ...
        <span className="rw-secactions">
          ...
          <button title="Open just this section as its own custom wall" onClick={()=>onSolo&&onSolo(sec)}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 3h6v6M21 3l-8 8M9 21H3v-6M3 21l8-8"/></svg></button>
        </span>
      </div>
      {!collapsed
...
function WallBgControl({ value, onApply, onReset }){
  const [open,setOpen]=useState(false);
  const CHECKER="repeating-conic-gradient(#cfd2da 0% 25%, #fff 0% 50%) 50% / 12px 12px";
  const [tShade,setTShade]=useState('#FFFFFF');
  const [tOp,setTOp]=useState(35);
  const tVal=(shade,op)=> shade==='__dark'?'rgba(20,19,30,'+(op/100)+')':'color-mix(in oklab, '+shade+' '+op+'%, transparent)';
  const apply=(b)=>{ onApply(b); setOpen(false); };
  return (
    <span className="rw-secbgwrap rw-wallbgwrap">
      <button className={'rw-iconbtn'+(value?' active':'')} title="Wall background — fully customize this wall" onClick={()=>setOpen(o=>!o)}>{W.image}</button>
      {open && <div className="rw-secbgpop rw-wallbgpop" onMouseLeave={()=>setOpen(false)} onClick={e=>e.stopPropagation()}>
        <div className="rw-bgtitle">Wall background</div>
        <button className={'rw-bgfollow'+(!value?' on':'')} onClick={()=>{ onReset(); setOpen(false); }}><span className="rw-bgfollow-ic">↺</span><span style={{minWidth:0}}><b>Follow page style</b><small>Uses the app’s Frame &amp; Background</small></span>{!value&&<span className="rw-bgfollow-ck">✓</span>}</button>
        <div className="rw-bglbl">Color</div>
        <div className="rw-bgcolors">{['#FFFFFF','#1C1B2E','#FFF6E6','#FFE9D6','#FCE4EF','#F3E6FB','#E6F1FF','#E3F6EE','#FBE9E7','#EEF1F4'].map(c=><button key={c} style={{background:c}} onClick={()=>apply({type:'color',value:c})}/>)}</div>
        <div className="rw-bglbl">Translucent — shade + opacity</div>
        <div className="rw-bgshades">{[['white','#FFFFFF'],['honey','#F4B740'],['sky','#5BA8FF'],['blossom','#E083B8'],['mint','#4FD7A6'],['dark','__dark']].map(([k,c])=><button key={k} title={k} className={'rw-bgshade'+(tShade===c?' on':'')} style={{background:c==='__dark'?'#14131e':c}} onClick={()=>setTShade(c)}/>)}</div>
        <div className="rw-bgoprow"><input type="range" min="10" max="85" value={tOp} onChange={e=>setTOp(+e.target.value)}/><span className="rw-bgpreview" style={{backgroundImage:'linear-gradient('+tVal(tShade,tOp)+','+tVal(tShade,tOp)+'), '+CHECKER}}/><span className="rw-bgopval">{tOp}%</span></div>
        <button className="rw-bgapply" onClick={()=>apply({type:'color',value:tVal(tShade,tOp)})}>Use translucent</button>
        <div className="rw-bglbl">Wash</div>
        <div className="rw-bgwashes">{[['--grad-dawn','Dawn'],['--grad-honey','Honey'],['--grad-mint','Mint'],['--grad-brand','Sky']].map(([g,n])=><button key={g} title={n} style={{background:'var('+g+')'}} onClick={()=>apply({type:'wash',value:g})}/>)}</div>
        <div className="rw-bglbl">Photo</div>
        <div className="rw-bgphotos">{['photos/p1.png','photos/p2.png','photos/p3.png'].map(p=><button key={p} style={{backgroundImage:"url('"+p+"')"}} onClick={()=>apply({type:'photo',value:p})}/>)}
          <label className="rw-bgupload" title="Upload a photo">+<input type="file" accept="image/*" style={{display:'none'}} onChange={(e)=>{ const f=e.target.files&&e.target.files[0]; if(f){ apply({type:'photo',value:URL.createObjectURL(f)}); } }}/></label>
        </div>
      </div>}
    </span>
  );
...
function SoloBoard({ sec, wallView, onAddItem, onMoveCard, onOpen, onEnlarge, onBoard, onModal, onAddSection, onClose }){
  const [view,setView]=useState(wallView||'icon');
  const [filter,setFilter]=useState('All');
  const [q,setQ]=useState('');
  const sbg=secBgGet(sec);
  const sbgStyle = sbg ? (
    sbg.type==='color' ? { background:sbg.value } :
    sbg.type==='wash' ? { background:'var('+sbg.value+')' } :
    sbg.type==='photo' ? { backgroundImage:"url('"+sbg.value+"')", backgroundSize:'cover', backgroundPosition:'center' } : {}
  ) : {};
  let items = sec.items||[];
  if(filter!=='All'){ const t=TYPE_FILTER[filter]; items=items.filter(r=>r.type===t); }
  if(q) items=items.filter(r=>(r.label||'').toLowerCase().includes(q.toLowerCase()));
  return (
    <div className="rw-soloscrim">
      <div className={'rw-solo'+(sbg?' has-bg':'')} style={{'--sc':cv(sec.color), ...sbgStyle}}>
        <div className="rw-solohead">
          <span className="rw-secdot" style={{background:cv(sec.color)}}/>
          <span className="rw-solotitle">{sec.title}</span>
          <span className="rw-solometa">{(sec.items||[]).length} resources</span>
          <div className="rw-solobar">
            <label className="rw-solosearch">{W.search}<input value={q} placeholder="Search…" onChange={e=>setQ(e.target.value)}/></label>
            <select className="rw-solofilter" value={filter} onChange={e=>setFilter(e.target.value)}>{FILTERS.map(f=><option key={f} value={f}>{f}</option>)}</select>
            <div className="rw-soloviews">{VIEWS.map(v=><button key={v[0]} className={view===v[0]?'on':''} title={v[1]} onClick={()=>setView(v[0])}>{v[2]}</button>)}</div>
          </div>
          <button className="rw-soloplay" title="Slideshow this section" onClick={()=>sec.items[0]&&onOpen(sec.items[0],sec.items)}>{W.play}</button>
          <button className="rw-soloclose" title="Close" onClick={onClose}>{W.x}</button>
        </div>
        <div className={'rw-grid v-'+view}>
          {items.map(r=><Card key={r.key} r={r} view={view} secId={sec.id} onDropBefore={onMoveCard} onOpen={(rr)=>onOpen(rr,sec.items)} onEnlarge={onEnlarge} onBoard={onBoard} onModal={onModal}/>)}
          <button className="rw-addcard" onClick={()=>onAddItem(sec.id)}>{W.plus}<span>Add resource</span></button>
        </div>
        <div className="rw-solofoot">
          <button className="rw-soloaddsec" onClick={()=>onAddSection(sec)}>{W.plus} Add section</button>
          <span className="rw-solofoothint">Adding a section turns this into a custom wall in My Walls, linked to this lesson.</span>
        </div>
      </div>
    </div>
  );
...
function ResourceWall({ state, focus, onTeach }){
  ...
  useEffect(()=>{ const id=window.__openWallId; if(id){ window.__openWallId=null; try{ const w=(JSON.parse(localStorage.getItem('cc_customwalls')||'[]')).find(x=>x.id===id); if(w){ loadLayout(w); setWallMode('custom'); } }catch(e){} } },[]);
  useEffect(()=>{ window.__rwCanBack = !!activeCustom; return ()=>{ try{ window.__rwCanBack=false; }catch(e){} }; },[activeCustom]);
  useEffect(()=>{ const h=()=>{ setWallMode('preset'); setActiveCustom(null); setSecs(buildSections(state)); }; window.addEventListener('cc-rw-back',h); return ()=>window.removeEventListener('cc-rw-back',h); },[]);
  ...
  const soloPromote=(sec)=>{
    const lessonId=(sec.id||'').replace(/^sec-/,'');
    const draftName=sec.title+' (working)';
    const newSec={ id:'sec-new'+(_uid++), title:'New section', meta:'', color:'--subj-9', subjectId:'spelling', items:[] };
    const layout=[
      { id:sec.id, title:sec.title, meta:sec.meta, color:sec.color, subjectId:sec.subjectId, items:sec.items },
      { id:newSec.id, title:newSec.title, meta:'', color:newSec.color, subjectId:newSec.subjectId, items:[] }
    ];
    const w={ id:'cw'+Date.now(), name:draftName, anchor:'lesson', lessonId, lessonLabel:sec.title+(sec.meta?(' · '+sec.meta):''), draft:true, layout, view, secCount:2, created:Date.now() };
    const next=[w, ...customWalls]; setCustomWalls(next); saveCustomWalls(next);
    setSecs(layout.map(x=>({ ...x, key:'s'+(_uid++), items:(x.items||[]).map(it=>({...it,key:it.key||('k'+(_uid++))})) })));
    setActiveCustom(w); setWallMode('custom'); setSoloSec(null);
    setToast('Created “'+draftName+'” in My Walls — linked to this lesson ✓');
  };
  const wallName = wallMode==='custom' ? (activeCustom?activeCustom.name:'Choose a wall') : preset;
  const wallKey = wallMode==='custom' ? (activeCustom?('cw:'+activeCustom.id):'cw:none') : ('preset:'+preset);
  const [wallBg,setWallBg]=useState(()=>{ try{ return JSON.parse(localStorage.getItem('cc_wallbg_'+wallKey)); }catch(e){ return null; } });
  useEffect(()=>{ try{ setWallBg(JSON.parse(localStorage.getItem('cc_wallbg_'+wallKey))); }catch(e){ setWallBg(null); } },[wallKey]);
  const applyWallBg=(b)=>{ setWallBg(b); try{ localStorage.setItem('cc_wallbg_'+wallKey, JSON.stringify(b)); }catch(e){} };
  const resetWallBg=()=>{ setWallBg(null); try{ localStorage.removeItem('cc_wallbg_'+wallKey); }catch(e){} };
  const wallBgStyle = wallBg ? (
    wallBg.type==='color' ? { background:wallBg.value } :
    wallBg.type==='wash' ? { background:'var('+wallBg.value+')' } :
    wallBg.type==='photo' ? { backgroundImage:"url('"+wallBg.value+"')", backgroundSize:'cover', backgroundPosition:'center' } : null
  ) : null;

  return (
    <div className={'viewbody rw-root'+(subjColor?' rw-subjcolor':'')+(wallBg?' rw-haswallbg':'')}>
      {wallBg && document.querySelector('.frame') && ReactDOM.createPortal(<div className="rw-framebg" style={wallBgStyle||undefined}/>, document.querySelector('.frame'))}
      <div className="rw-bar">
            ...
            <div className="rw-switchlist">
              {switchTab==='presets'
                ...
                : (customWalls.length? customWalls.slice(0,8).map(w=>(
                    <button key={w.id} className={activeCustom&&activeCustom.id===w.id?'on':''} onClick={()=>{ loadLayout(w); setWallMode('custom'); setSwitchOpen(false); }}>{activeCustom&&activeCustom.id===w.id?W.check:<span className="sp"/>}{w.name}{w.anchor==='lesson'&&w.lessonLabel?<span className="rw-custlink" title={'Linked to '+w.lessonLabel}>{W.link}{w.lessonLabel}</span>:<span className="rw-custanchor">{w.layout?(w.secCount+' sec'):w.anchor}</span>}</button>
        ...
        <WallBgControl value={wallBg} onApply={applyWallBg} onReset={resetWallBg} />
      ...
      {soloSec && (()=>{ const s=secs.find(x=>x.id===soloSec); if(!s) return null;
        return <SoloBoard key={s.id} sec={s} wallView={view} onAddItem={addItemP} onMoveCard={moveCardP}
          onOpen={openLight} onEnlarge={enlarge} onBoard={board} onModal={setModal}
          onAddSection={soloPromote} onClose={()=>setSoloSec(null)} />; })()}
