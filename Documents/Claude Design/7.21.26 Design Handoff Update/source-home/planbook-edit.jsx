...
/* resource chips carry their data so they can be opened/edited from anywhere */
const RES_ICONS={Image:'🖼',Link:'🔗',Note:'📝',Slides:'📑',Doc:'📄',Video:'🎬',Worksheet:'📝',Drive:'▲',File:'📎'};
const escAttr=(s)=>String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
function chipHTML(res){ const type=res.type||'Link'; const icon=RES_ICONS[type]||'📎';
  const name=res.name||res.title||res.label||'Resource'; const url=res.url||''; const note=res.note||res.body||'';
  return '<span class="pb-rchip" contenteditable="false" data-type="'+escAttr(type)+'" data-name="'+escAttr(name)+'"'
    +(url?' data-url="'+escAttr(url)+'"':'')+(note?' data-note="'+escAttr(note)+'"':'')+'>'+icon+' '+escAttr(name)+'</span>'; }
function chipRes(chip){ return { type:chip.getAttribute('data-type')||'Link',
  name:chip.getAttribute('data-name')||chip.textContent.replace(/^\S+\s*/,'').trim()||'Resource',
  url:chip.getAttribute('data-url')||'', note:chip.getAttribute('data-note')||'' }; }
...
function SelectionToolbar(){
  ...
  useEffect(()=>{
    ...
    const onChipClick=(e)=>{ const chip=e.target.closest&&e.target.closest('.pb-rchip'); if(!chip) return;
      e.preventDefault(); e.stopPropagation();
      const res=chipRes(chip); const field=chip.closest('.pb-field');
      const commitField=()=>{ if(field) field.dispatchEvent(new Event('input',{bubbles:true})); };
      if(window.openResMenu){ window.openResMenu({ res, x:e.clientX, y:e.clientY,
        edit: ()=>{ if(!window.openComposer){ if(res.url) window.open(res.url,'_blank','noopener'); return; }
          window.openComposer({ lessonTitle:'This lesson', edit:res, onSave:(list)=>{ const f0=list[0]; if(!f0) return;
            chip.setAttribute('data-type',f0.type||'Link'); chip.setAttribute('data-name',f0.name||'Resource');
            if(f0.url) chip.setAttribute('data-url',f0.url); else chip.removeAttribute('data-url');
            if(f0.note) chip.setAttribute('data-note',f0.note); else chip.removeAttribute('data-note');
            chip.textContent=(RES_ICONS[f0.type]||'📎')+' '+(f0.name||'Resource'); commitField(); } }); },
        remove: ()=>{ const nx=chip.nextSibling; chip.remove(); if(nx&&nx.nodeType===3&&/^[\u00a0\s]*$/.test(nx.textContent)) nx.remove(); commitField(); } });
    ...
    return ()=>{ document.removeEventListener('click',onChipClick,true); document.removeEventListener('selectionchange',onSel); document.removeEventListener('focusin',onFocusIn); document.removeEventListener('focusout',onFocusOut); };
  },[]);
  ...
  return ReactDOM.createPortal(
    <div className="pb-fbar pb-portal" style={{left, top:st.y}} onMouseDown={e=>e.preventDefault()}>
      ...
      <button className="pb-tb" title="Resource chip — add a resource or note" onClick={()=>{ if(!window.openComposer){ const nm=prompt('Resource name:','Slides'); if(nm) insert(chipHTML({name:nm,type:'File'})+'&nbsp;'); return; }
        let rng=null; const sel=document.getSelection(); if(sel&&sel.rangeCount) rng=sel.getRangeAt(0).cloneRange();
        window.openComposer({ onSave:(list)=>{ if(rng){ const s2=document.getSelection(); s2.removeAllRanges(); s2.addRange(rng); } list.forEach(it=>{ insert(chipHTML(it)+'&nbsp;'); }); } }); }}>📎</button>
    </div>, pbHost());
}
...
function migrateSecColors(arr, L){
  ...
  let resHtml=null;
  const buildRes=()=>{ if(resHtml!=null) return resHtml; try{ resHtml=(DS.resourcesFor(L)||[]).slice(0,4).map(r=>chipHTML(r)).join('&nbsp;'); }catch(e){ resHtml=''; } return resHtml; };
...
function loadSections(L){
  ...
  let resHtml='';
  try{ resHtml=(DS.resourcesFor(L)||[]).slice(0,4).map(r=>chipHTML(r)).join('&nbsp;'); }catch(e){}
...
function LessonSections({ L, readOnly }){
  ...
  const addRes=(secId,res)=>{ const chip=chipHTML(res)+'&nbsp;';
      ...
      {secs.map((s,idx)=>(
        <div className={'pb-sec'+(dropIdx===idx?' drop':'')+(settleId===s.id?' settling':'')+(s.tintText?' tinted':'')} key={s.id} style={{'--rc':s.color||sc(L.subjectId)}}>
          ...
          {!readOnly && (s.label||'').trim().toLowerCase().includes('resource') && activeSec===s.id && (
            <div className="pb-stdadd" onMouseDown={e=>e.preventDefault()}>
              <button className="pb-tplbtn" onClick={()=>{ if(!window.openComposer){ setResMenuFor(o=>o===s.id?null:s.id); return; }
                window.openComposer({ subject:L.subjectId, lessonTitle:L.title||'This lesson', sectionLabel:s.label,
                  onSave:(list)=>list.forEach(it=>addRes(s.id,{title:it.name||'Resource',type:it.type,url:it.url,body:it.note})) }); }}>+ Add resource or note</button>
            </div>
          )}
        </div>
      ))}
...
/* lesson cell (board) */
function Cell({ L, dayKey, expanded, onToggle, onOpen, drag, onBusy, register, min, onDrag, onSubject }){
  return (
    <div className={'pb-cell'+(L.status==='now'?' now':'')+(min?' min':'')} style={{'--rc':sc(L.subjectId)}} ref={el=>register&&register(L.id, el)}>
      <div className="pb-cellh" draggable style={{background:sc(L.subjectId), cursor:'grab'}} title="Drag to move · click to expand / collapse"
        ...
        {onSubject && <button className="pb-weekchev" title={'Expand '+slabel(L.subjectId)+' across the whole week — click again to collapse'}
          onClick={(e)=>{ e.stopPropagation(); onSubject(L.subjectId); }}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"><path d="M5 7l7 5 7-5M5 13l7 5 7-5"/></svg>
        </button>}
...
function SlotBoard({ days, board, expanded, toggle, onOpen, drag, register, busy, onBusy, over, onOver, commit, onAdd, layout, dragId, onDrag, onSubject }){
            ...
            <div key={d.key+(stacked?ri:p.key)} className={'pb-slot'+(items.length?'':' empty')+(isOver?' over':'')}
              ...
              {items.length ? items.map(L=>(
                <Cell key={L.id} L={L} dayKey={d.key} expanded={!!expanded[L.id]}
                  onToggle={()=>toggle(L.id)} onOpen={onOpen} drag={drag} onBusy={onBusy} register={register} min={dragId===L.id} onDrag={onDrag} onSubject={onSubject} />
...
function WeekEdit({ state, mode, onOpen }){
  ...
  const [expanded,setExpanded]=useState(()=>{ try{ return JSON.parse(localStorage.getItem('cc_week_expand'))||{}; }catch(e){ return {}; } });
  React.useEffect(()=>{ try{ localStorage.setItem('cc_week_expand',JSON.stringify(expanded)); }catch(e){} },[expanded]);
  ...
  const allIds=(pred)=>Object.values(board).flat().filter(pred||(()=>true)).map(l=>l.id);
  const toggleSubject=(sid)=>{ const ids=allIds(l=>l.subjectId===sid);
    setExpanded(p=>{ const anyClosed=ids.some(id=>!p[id]); const n={...p}; ids.forEach(id=>{ n[id]=anyClosed; }); return n; }); };
  const setAll=(on)=>setExpanded(()=>{ const n={}; if(on) allIds().forEach(id=>{ n[id]=true; }); return n; });
  ...
  return (
    <div className="pb-editwrap">
      <div className="pb-weektools">
        <span className="pb-wthint">Tip: the ⌄⌄ on a card opens that subject’s whole week</span>
        <button title="Expand every lesson this week" onClick={()=>setAll(true)}>Expand all</button>
        <button title="Collapse every lesson this week" onClick={()=>setAll(false)}>Collapse all</button>
      </div>
      <SlotBoard days={days.map((d,i)=>({ ...d, _today: i===state.todayIdx }))} board={board}
        ...
        busy={busy} onBusy={setBusy} over={over} onOver={onOver} commit={commitDrop} onAdd={onAdd} layout={layout} dragId={dragId} onDrag={setDragId} onSubject={toggleSubject} />
    </div>
  );
}
