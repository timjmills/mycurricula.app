...
function LessonNav({ state, tab='lessons', onTab, onPickLesson, onPickResource, onPlanLesson, onPostLesson, onClose, activeId, activeLesson, version, showForking, onTeach, hideTabs, onPresentResource, onToBoard }){
  ...
  return (
    <div className="lnav">
      {!hideTabs && <div className="lnav-tabs">
      ...
      {tab==='lessons' && planning
        ? <div className="lnav-plan">
            ...
            {window.PBEdit && window.PBEdit.LessonEditor
              ? <window.PBEdit.LessonEditor L={planning} onTeach={(l)=>{ if(onTeach)onTeach(l||planning); setPlanning(null); }} />
              : (window.PlanPage
            ...
            {g.items.map((it,ii)=>{
              ...
              return (
                ...
                {tab!=='resources' && <span className="lnav-rowacts">
                  ...
                  {onToBoard && <button title="Send the objective to the board as editable text" onClick={()=>onToBoard(it)}>→Board</button>}
                </span>}
                {tab==='resources' && onPresentResource && <span className="lnav-rowacts">
                  <button title="Present — fill the board with this resource" onClick={()=>onPresentResource(it)}>Present</button>
                </span>}
