...
- **Master mode**: shows only master content. Editing the **Team Curriculum** is entered via the **Personal | Team** toggle and signaled by the **pink caution glow** (`#E8179B` on `[data-mode="team"]`) — there is no separate "Now editing master" confirm button; the glow IS the safety.
...
- First master-mode entry: one-shot tooltip explaining the **Personal | Team** Curriculum toggle and the pink caution glow that signals Team-Curriculum editing.
...
- [ ] Master-mode banner is unmissable; editing the Team Curriculum is signaled by the pink caution glow (`#E8179B` on `[data-mode="team"]`) with no confirm dialog — the glow is the safety. (Banner sequence shipped in Phase 1; this verifies the full edit flow works on top.)
...
| **Editing master** | The explicit mode required to make changes to the master plan. Editing the Team Curriculum is signaled by the pink caution glow; no confirmation dialog. |
