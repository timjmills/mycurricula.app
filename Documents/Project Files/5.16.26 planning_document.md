...
>
> **⏳ Pending build TODO (2026-06-14): ADD PHOTOGRAPHIC IMAGERY to the teacher home (`/home`) and the public marketing landing (`/welcome`).** Both surfaces are built and ship today with a themed gradient wash / placeholder veil; the candid teacher photos are still to be generated via ChatGPT / the `gen-image` skill (`/home` → `public/home/*.webp`; `/welcome` → `public/*.webp` hero + 1200×630 og-image) once the ChatGPT/Codex usage limit resets (~Jun 18, 2026). Tracked in `docs/landing-home-checklist.md`; this is the only outstanding item for those two pages.
