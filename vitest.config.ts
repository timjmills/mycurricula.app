...
//   • lib/sanitize-html.ts  — DOMPurify-backed; runs under node via jsdom.
...
//
// `environment: "node"` is sufficient: isomorphic-dompurify pulls in jsdom on
// its own, so we don't need vitest's jsdom environment. The `@/` path alias is
