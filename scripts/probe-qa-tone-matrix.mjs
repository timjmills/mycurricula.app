...
const AXES = {
  ...
  "photo-dim": { frame: "glass", glass: "dark", bg: "photo", theme: "clear", dim: "dim", tone: "dark" },
  "photo-bright": { frame: "glass", glass: "dark", bg: "photo", theme: "clear", dim: "bright", tone: "light" },
  night: { frame: "glass", glass: "dark", bg: "photo", theme: "night", dim: "normal", tone: "dark" },
};
const cookieValue = (a) => `v1.${a.frame}.${a.glass}.${a.bg}.${a.theme}.${a.dim}.vivid.highlight`;
...
const record = (v, name, detail = "", extra = {}) => {
  ...
  const tag = { PASS: "PASS ", FAIL: "FAIL ", ABSENT: "ABSNT", UNVERIFIED: "UNVER" }[v];
...
const RESOLVER = () => {
  Object.assign(window, {
    __resolveRGBA(css) {
      ...
      const rgb = alpha > 0.004 ? onBlack.map((v) => Math.min(255, v / alpha)) : [0, 0, 0];
...
async function measureText(page, target, label) {
  ...
  const info = await page.evaluate((t) => {
    ...
    if (r.width < 3 || r.height < 3) return { tooSmall: true, w: r.width, h: r.height };
  ...
  if (info.tooSmall) return { status: "absent", label, why: `box ${info.w}×${info.h}` };
  ...
  const y1 = Math.max(0, Math.min(vp.height, info.rect.y + info.rect.height - 1));
  const clip = { x: x0, y: y0, width: x1 - x0, height: y1 - y0 };
  if (clip.width < 2 || clip.height < 2)
    return { status: "absent", label, why: `clip ${clip.width}×${clip.height} after viewport clamp` };
  ...
  try {
    ...
    return { status: "absent", label, why: `screenshot failed: ${String(e.message).split("\n")[0]}` };
  } finally {
  ...
  return {
    ...
    bg: bd.median,
    spread: [Math.round(lum(bd.p10) * 1000) / 1000, Math.round(lum(bd.p90) * 1000) / 1000],
  };
}
...
async function makeContext(browser, axisKey, viewport) {
  ...
  ctx.on("request", (r) => {
    if (r.url().includes("/rest/v1/")) rest.push(r.url().split("/rest/v1/")[1].split("?")[0]);
  });
  ...
  await ctx.addCookies([{ name: "mc-theme-axes", value: cookieValue(a), url: BASE }]);
  await bypassLogin(ctx, { base: BASE, next: "/weekly", retries: 2, timeout: 180000 });
  return { ctx, rest };
}
...
const SURFACES = [
  ...
  },
  {
    ...
    open: async (page) => {
      ...
      await page.evaluate(() => document.querySelector("button[data-year-chip]")?.click());
      await page.waitForSelector('[role="dialog"][aria-modal="true"]', { timeout: 60000 });
      await page.waitForSelector('[data-ue-tab="refine"]', { timeout: 30000 });
      await page.evaluate(() => document.querySelector('[data-ue-tab="refine"]')?.click());
    },
    ...
    text: [
      ...
      ["row meta (Wk N · Day)", { sel: '#ue-tabpanel [class*="RefineTab_rowMeta__"]' }],
      ["pass progress status", { sel: '#ue-tabpanel [class*="RefineTab_passProgress__"]' }],
      ["empty-cell placeholder", { sel: '#ue-tabpanel [class*="RefineTab_placeholder__"]' }],
      ["dot completeness count", { sel: '#ue-tabpanel [class*="RefineTab_dotCount__"]' }],
    ],
  },
  {
    id: "teach-header",
  ...
  },
  {
    ...
    text: [
      ...
      ["board-cap meter label", { sel: '[class*="BoardLibrary_meterLabel__"]' }],
  ...
  },
  {
    ...
    text: [
      ["Post pill label", { sel: '[data-planner-item^="lesson:"] [class*="day-v2_vaPillBtn"]', text: "^Post$" }],
      ["Plan pill label", { sel: '[data-planner-item^="lesson:"] [class*="day-v2_vaPillBtn"]', text: "^Plan$" }],
      ["Teach pill label", { sel: '[data-planner-item^="lesson:"] [class*="day-v2_vaPillBtn"]', text: "^Teach$" }],
    ...
    touch: [["Post pill", '[data-planner-item^="lesson:"] [class*="day-v2_vaPillBtn"]', "^Post$"]],
  },
  {
    id: "post-chips",
    ...
    root: "[data-sectags]",
    text: [
      ["tag chip (KNOWN 48% divergence)", { sel: '[data-sectags] > span[role="listitem"]' }],
      ["tag chip label", { sel: '[data-sectags] [class*="Section_tagLabel__"]' }],
      ["+N overflow pill", { sel: '[data-sectags] button[class*="Section_tagMore__"]' }],
...
const SUBJECT_SOLIDS = async (page) =>
  page.evaluate(() => {
    ...
    const surface = window.__resolveRGBA(cs.getPropertyValue("--surface").trim() || "#fff");
    const out = [];
...
async function runSurface(page, s, axisKey, width, restCounter) {
  ...
  try {
    await page.goto(`${BASE}${s.goto}`, { waitUntil: "domcontentloaded", timeout: 180000 });
  } catch (e) {
    record("ABSENT", `${tag} — navigation`, redact(String(e.message).split("\n")[0]));
    ...
    await page.waitForFunction(
      () => {
        ...
        for (let i = 0; i < nodes.length; i += 1)
          for (const k in nodes[i]) if (k.startsWith("__reactFiber$")) return true;
        return false;
      },
      null,
  ...
  if (!hydrated) {
    ...
    record("UNVERIFIED", `${tag} — hydration gate`, "React never attached in 240s (dev-server contention)");
    page.off("console", onMsg);
    return null;
  }
  ...
  if (!tone) {
    record("UNVERIFIED", `${tag} — abandoned`, "tone gate failed; measurements would be mislabelled");
    page.off("console", onMsg);
    return null;
  }
  ...
  if (s.open) {
    try {
      ...
      record("ABSENT", `${tag} — could not open`, redact(String(e.message).split("\n")[0]));
      page.off("console", onMsg);
      return null;
    }
  }
  ...
  if (!present) {
    record("ABSENT", `${tag} — surface never rendered`, `root ${s.root} not visible in 90s`);
    await page.screenshot({ path: path.join(SHOTS, `${s.id}-${axisKey}-${width}-ABSENT.png`) }).catch(() => {});
    page.off("console", onMsg);
    return null;
  }
  if (s.settle) await page.waitForSelector(s.settle, { timeout: 30000 }).catch(() => {});
  ...
  record(scrollX <= 1 ? "PASS" : "FAIL", `${tag} — no document h-scroll`, `${scrollX}px`);
  ...
  for (const [label, target] of s.text) {
    ...
    measured.push({ ...m, tag, axisKey, width, surface: s.id, tone: tone.tone });
    const passAA = m.ratio >= m.floor;
    record(
      passAA ? "PASS" : "FAIL",
      `${tag} — ${label} AA (floor ${m.floor})`,
  ...
  if (width === 375 && s.touch) {
    for (const [label, sel, text] of s.touch) {
      ...
      if (!box) record("ABSENT", `${tag} — ${label} touch target`, "control not found");
      else
        record(
          box.h >= 44 ? "PASS" : "FAIL",
  ...
  if (consoleErrors.length) {
    record("FAIL", `${tag} — browser console clean`, `${consoleErrors.length}: ${consoleErrors[0]}`);
  } else {
...
async function main() {
  ...
  for (const [axisKey, width] of PLAN) {
    ...
    try {
      ...
      record("UNVERIFIED", `${axisKey}@${width} — context/auth`, redact(String(e.message).split("\n")[0]));
      continue;
    }
    ...
    if (!subjectReport || !subjectReport[AXES[axisKey].tone]) {
      ...
      if (sub) {
        ...
        const fails = sub.solids
          ...
          .sort((a, b) => a.r - b.r);
        subjectReport[AXES[axisKey].tone] = { surface: sub.surface, ratios: fails };
    ...
    console.log(`  [/rest/v1/ requests seen: ${rest.length} -> ${[...new Set(rest)].join(", ") || "none"}]`);
    await ctx.close();
  }
  await browser.close();
  ...
  writeFileSync(
    ...
    JSON.stringify({ head: HEAD, dirty: !!DIRTY, base: BASE, rows, measured: all, subjectReport, restSeen: [...restSeen] }, null, 2),
  );
  ...
  console.log(`   /rest/v1/ endpoints touched: ${[...restSeen].join(", ") || "NONE (mock planner path)"}`);
...
async function selftest() {
  ...
  await ctx.addInitScript(() => {
    localStorage.setItem("mycurricula:onboarding", JSON.stringify({ stepIndex: 0, data: {}, finished: true }));
  ...
  await ctx.addCookies([{ name: "mc-theme-axes", value: cookieValue(AXES["photo-dim"]), url: BASE }]);
  await bypassLogin(ctx, { base: BASE, next: "/weekly", retries: 2, timeout: 180000 });
  const page = await ctx.newPage();
  ...
  // returning a number.
  const miss = await measureText(page, { sel: "#definitely-not-here" }, "control");
  console.log(`  ${miss.status === "absent" ? "ok  " : "FAIL"} measureText returns ABSENT, not a ratio, for a missing element`);
  ...
  console.log(`  ${hit.status === "ok" && hit.ratio > 0 ? "ok  " : "FAIL"} positive control: <body> measures (${hit.ratio?.toFixed(2)}:1)`);
