...
function OverviewBody(): ReactNode {
  ...
  // Standards lives in Supabase (not localStorage), so fetch a live one-liner
  // for the overview tile — the count of the teacher's effective frameworks.
  const [standardsSummary, setStandardsSummary] = useState<string>(
    "Choose your curriculum frameworks",
  );
  useEffect(() => {
    let alive = true;
    fetch("/api/standards/frameworks")
      .then((r) => r.json())
      .then((d: { effectiveIds?: unknown }) => {
        if (!alive) return;
        const n = Array.isArray(d.effectiveIds) ? d.effectiveIds.length : 0;
        setStandardsSummary(
          n > 0
            ? `${n} ${n === 1 ? "framework" : "frameworks"} active`
            : "Choose your curriculum frameworks",
        );
      })
      .catch(() => {
