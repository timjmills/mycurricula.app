...
> `pathname`, `user_agent`, `reason`, `created_at` — with `reason` **NULL on a
> successful bypass**. The table is now codified in
> `supabase/migrations/20260607120000_claude_access_log_reconcile.sql`; the
> canonical way to (re)create or repair it is `supabase db push` (idempotent).
>
> If you are repairing a hand-created table that drifted to the old
> `success`/`path` shape (inserts silently fail, because audit-log writes are
> non-fatal by design), the migration fixes it — or apply the equivalent by hand.
> **The renames alone are NOT enough**: `reason` must be made NULLABLE (success
> rows carry no reason — this was the bug that suppressed every success row), and
> PostgREST's schema cache must be reloaded (the app inserts via
> supabase-js → PostgREST, which caches column names; without the reload, app
> inserts keep failing on the renamed columns even though the table is correct):
>
> ```sql
> alter table public.claude_access_log rename column success to ok;        -- if old shape
> alter table public.claude_access_log rename column path    to pathname;  -- if old shape
> alter table public.claude_access_log alter column reason drop not null;  -- success rows log reason=NULL
> alter table public.claude_access_log add column if not exists user_agent text;
> notify pgrst, 'reload schema';  -- REQUIRED: refresh PostgREST's cached column list
> ```
>
> Reads are **owner-only and SELECT-only** — the `claude_admin_read` RLS policy
> `for select … using (public.is_claude_admin())`. SELECT-only keeps this
> append-only audit log tamper-proof from app-side credentials (only the service
> role writes). The broad `authenticated selects USING (true)` from earlier docs
> is **retired**: now that multi-tenant provisioning is live it would leak the
> owner's Claude telemetry to every authenticated teacher.
