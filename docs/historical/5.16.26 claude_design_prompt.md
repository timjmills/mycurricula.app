...
48. **Bump cascade preview modal (Phase 1B)** — when a teacher clicks Bump forward/backward on a lesson, show a modal listing every downstream lesson that will shift, with old → new dates clearly marked. Locked events shown crossed-out with "Locked — will be skipped." Confirm or Cancel. Quiet only.
49. **Lock to date affordance (Phase 1B)** — small lock icon on lesson cards in Advanced mode; click to lock/unlock. Locked lessons display the lock icon persistently in a corner of the card. Quiet only.
50. **Reminders Panel right-rail (Phase 2)** — collapsible sidebar with 7-day lookahead: section per day showing to-dos, assessments, repeating to-dos, events, birthdays. Collapsed state is a thin vertical strip with item-count badges; expanded state is ~280px wide. Quiet only.
51. **Digital sticker affordance (Phase 2)** — sticker picker shown via right-click → "Add sticker" on a calendar day; small icon library; stickers render on calendar cells without obscuring lesson content. Show 3-4 sample stickers across a Weekly grid. Quiet only.
52. **Class website / parent-share enable flow (Phase 2)** — Settings → Sharing panel with a single toggle "Public class page," tokenized URL display, content scope controls (which sections show? which date range? resource links visible?), embed code snippet, and "Open public page in new tab" preview button. Quiet only.
53. **Public class page render (Phase 2)** — what parents actually see at the tokenized URL. Read-only view; clean typography; week of lessons with the teacher-selected sections visible; no edit affordances, no personal indicators. Quiet only.
54. **Standards coverage report (Phase 2)** — full-page report: filter chips at top (subject, framework, date range, unit); table or list of standards with code, description, lesson count, expand-to-see-lessons. Sortable. Untagged section at bottom. Export button. Quiet only.
55. **PWA install prompt modal (Phase 1B)** — first-mobile-login overlay with title "Want this on your iPad's home screen?" and step-by-step screenshots showing Add to Home Screen flow. Platform-specific (iOS Safari vs Android Chrome variants). "Not now" and "Already installed" buttons. Quiet only.
56. **Top-bar full-text search (Phase 1B)** — search input in the top bar with `/` keyboard shortcut; results dropdown showing lesson title, subject color stripe, date, and snippet with highlighted match term. Source-type chips on Phase 2 expansion (Lesson / Comment / To-do / Note / Resource). Quiet only.
57. **Lesson Status Tag pills on lesson cards (Phase 2)** — small pill on the lesson card showing `Draft / Ready / Approved / Taught` with distinct color treatment. Visible only in Advanced mode (Simple mode hides). Show one card of each state side by side for the design. Quiet only.
58. **Class website public page render (Phase 2)** — view-only public page showing the week of lessons with the teacher's selected sections visible, optional header note above the grid, and the status-based filter applied. Clean typography, no edit affordances. Show example with header note populated. Quiet only.
59. **Lesson personal note affordance (Phase 2)** — small private-note icon on lesson cards when a note exists; inline expansion in detail view showing the note text with edit button. Quiet only.
60. **Curriculum bundle send + receive flow (Phase 3)** — multi-step modal: select scope (subjects/units/lessons), name the bundle, choose recipient (named colleague or export file). Inbox panel for receiving teacher showing pending bundles with accept/decline + merge/create-new option on accept. Quiet only.
61. **Team Lesson Bank panel (Phase 3)** — slide-out panel showing the department's banked lessons grouped by tag; each entry has name, tags, original author, usage count, drag handle. Filter chips at top. Quiet only.
62. **Plan Submission Report admin view (Phase 3)** — admin section table showing roster with each teacher's submission status for current and next deadline windows, status color coding, and bulk-message bar at top. Quiet only.
63. **`✏ N modified` badge on Master lesson cards (Phase 2)** — small badge on Master lesson cards in any view when one or more teammates have shared modifications. Hidden in Simple mode. Click opens the Modifications Browser panel. Quiet only.
64. **Modifications Browser slide-out panel (Phase 2)** — right-side slide-out listing each shared modification: teammate avatar + name, modification date + share date, auto-generated change summary, expand-on-click for diff view. Each row has three action buttons: Pull into my Personal / Promote to Master / Add to Team Lesson Bank. Quiet only.
65. **Diff view inside modifications panel (Phase 2)** — line-level highlighting (insertions in green, deletions in red, moves in blue) showing what the teammate changed vs the current Master. Reuses the Phase 4 curriculum-diff renderer; this is the first production use of it. Quiet only.
66. **"Share with team" toggle on Personal-copy lessons (Phase 2)** — small toggle in the lesson detail panel that appears on Personal copies with modifications. Tooltip explains "Toggling on lets teammates discover and reuse this modification." Quiet only.
67. **Global "Share by default" Setting (Phase 2)** — Settings → Profile → Sharing checkbox "Share my modifications with the team by default." Off by default. Helper text explains the privacy posture. Quiet only.
68. **Promote-to-Master confirmation modal (Phase 2)** — reuses the existing master-mode-entry banner sequence as a safety beat ("Promoting to Master will affect the whole team. Continue?") plus an extra confirmation step before changes propagate. Quiet only.
