...
// util.ts — the NON-COMPONENT members of the shared v2 planner atoms (a hook +
// two constants/helpers). Kept OUT of atoms.tsx (which exports only React
// components) so atoms stays a clean Fast-Refresh boundary: a module that mixes
// component and non-component exports can transiently undefine the
// non-components on a hot edit, which crashed the frames in dev (QA root-cause).
// lib/day-status.ts stays purely non-React; this file is where the shared
// canvas React-adjacent helpers (a client hook + render helpers) live.
//
// Lifted from components/day-v2/util.ts in Wave 5 so both the Day frames and
// the Week frames consume ONE copy (the day-v2 module now re-exports these).
