...
export function StandardsPicker({
  ...
  // Did the current pointer-press start on the backdrop itself? Guards
  // click-to-close so a text-selection drag that ends on the backdrop does
  // not discard the draft (review L-2).
  ...
  useEffect(() => {
    ...
    // Capture the opener BEFORE we move focus into the dialog. The search
    // input below grabs focus on open, so reading activeElement afterwards
    // would capture the search box (which unmounts on close) and lose the
    // real restore target — focus would fall to <body>. Restore focus to
    // this element when the dialog closes.
    const trigger =
  ...
  // Escape closes from anywhere while open. The dialog's own onKeyDown only
  // fires when focus is inside it, but focus can land on <body> after a
  // pointer interaction — a document-level listener makes Esc reliable.
  useEffect(() => {
    if (!open) return undefined;
  ...
  const restGroups = useMemo(() => {
    ...
    for (const fw of rest) {
      const label = REGION_LABELS[fw.region] ?? "Other regions";
  ...
  // Tagged codes that aren't in any bundled set (school-specific / DB
  // frameworks) — surfaced so they can be untagged here, and counted by the
  // empty-state note so it never contradicts a populated "other" list
  // (review L-4).
  const otherTagged = useMemo(() => {
    const known = new Set(
      Object.values(STANDARD_ITEMS).flatMap((items) =>
        items.map((it) => it.code),
      ),
    );
    return selected.filter((c) => !known.has(c));
  }, [selected]);
  ...
  if (!open) return null;
  // Portaled to <body> below so the fixed overlay escapes any display:none
  // ancestor — the Daily Dock collapses non-active columns at narrow widths,
  // which would otherwise drop this whole subtree (and the modal) from layout.
  // SSR-safe: `open` only flips true on a client click, so document exists.
        ...
        <div className={styles.fwRow}>
          ...
          <span className={styles.fwSide}>
            <Badge variant={licenceVariant(fw.commercial_use)} size="sm">
              {COMMERCIAL_USE_LABELS[fw.commercial_use] ?? "Licence unverified"}
            </Badge>
            <span
  ...
  return createPortal(
    <div
      ...
      onClick={(e) => {
        ...
        // a drag that began inside the dialog (e.g. selecting text) and
        // released here must not discard the draft (review L-2).
